...
public class TopicPartitionWriter {
  ...
  private boolean checkRotationOrAppend(
   ...
   try {
      isCompatible = compatibility.shouldChangeSchema(record, null, currentValueSchema);
    }catch(RuntimeException e){
       if(compatibility != StorageSchemaCompatibility.NONE)
       {
          throw e;
       }
      log.info("Got exception in compatibility check, ignoring : "+e.getMessage(),e);
      isCompatible = false;
    }
    if (isCompatible && recordCount > 0) {
